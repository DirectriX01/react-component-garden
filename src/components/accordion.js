...
function Accordion({ items}) {
    ...
    const [activeIndex, setActiveIndex] = useState(Array(items.length).fill(false));
    ...
    const renderedItems = items.map((item, index) => {
        ...
        return (
            <div key={item.id}>
            <div className="flex justify-between p-3 bg-gray-50 border-b items-center cursor-pointer" onClick={ () => onTitleClick(index)}>
                {item.title}
                {icon}
            </div>
